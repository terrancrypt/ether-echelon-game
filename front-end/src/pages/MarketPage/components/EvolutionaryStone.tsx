...
const EvolutionaryStone = () => {
  ...
  const fetchData = async () => {
    setData(stonesData as any);

    const arrTokenIds: any = [];
    Promise.all(
      Object.keys(stonesData).map((key) => {
    ...
    for (let i = 0; i < dataOnChain[0].length; i++) {
      dataArr.push({
        ...
        name: stonesData[formatUnits(dataOnChain[0][i], 0)].name,
        image: stonesData[formatUnits(dataOnChain[0][i], 0)].image,
...
export default EvolutionaryStone;
