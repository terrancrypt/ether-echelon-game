...
const Chests = () => {
  ...
  const fetchData = async () => {
    setData(chestsData as any);

    const arrTokenIds: any = [];
    Promise.all(
      Object.keys(chestsData).map((key) => {
    ...
    for (let i = 0; i < dataOnChain[0].length; i++) {
      dataArr.push({
        ...
        name: chestsData[formatUnits(dataOnChain[0][i], 0)].name,
        image: chestsData[formatUnits(dataOnChain[0][i], 0)].image,
